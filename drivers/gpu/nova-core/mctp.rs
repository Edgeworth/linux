...
impl TryFrom<Bounded<u32, 8>> for NvdmType {
    type Error = u32;

    fn try_from(value: Bounded<u32, 8>) -> Result<Self, Self::Error> {
        match *value {
            x if x == u8::from(Self::Cot).into() => Ok(Self::Cot),
            x if x == u8::from(Self::FspResponse).into() => Ok(Self::FspResponse),
            x => Err(x),
        }
    }
}
...
impl From<NvdmType> for Bounded<u32, 8> {
    fn from(value: NvdmType) -> Self {
        match value {
            NvdmType::Cot => Bounded::<u32, 8>::new::<0x14>(),
            NvdmType::FspResponse => Bounded::<u32, 8>::new::<0x15>(),
        }
    }
}
...
impl MctpHeader {
    ...
    pub(crate) fn single_packet() -> Self {
        Self::from_raw(0).with_som(true).with_eom(true)
    }
...
impl NvdmHeader {
    ...
    pub(crate) fn new(nvdm_type: NvdmType) -> Self {
        Self::from_raw(0)
            .with_msg_type(Bounded::<u32, 7>::new::<0x7e>())
    ...
    pub(crate) fn validate(self, expected_type: NvdmType) -> bool {
        *self.msg_type() == MSG_TYPE_VENDOR_PCI.into()
            && *self.vendor_id() == Vendor::NVIDIA.as_raw().into()
